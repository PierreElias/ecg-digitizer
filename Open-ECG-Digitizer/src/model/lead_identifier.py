...
class LeadIdentifier:
    ...
    def _select_layout_by_row_count(self, rows: int, layouts: dict[str, Any]) -> str:
        """Select most likely layout based on detected row count when matching fails.

        This provides a smarter fallback than alphabetical ordering. When lead label
        detection fails, we use the number of detected signal rows to guess the layout.

        Args:
            rows: Number of detected signal rows (merged lines)
            layouts: Available layout definitions

        Returns:
            Name of the best-matching layout
        """
        # Priority mapping: rows → preferred layouts (most common first)
        row_preferences: dict[int, list[str]] = {
            3: ["standard_3x4", "precordial_3x2", "standard_3x1"],
            4: ["standard_3x4_with_r1"],
            5: ["standard_3x4_with_r2"],
            6: ["standard_3x4_with_r3", "standard_6x2", "cabrera_6x1_limb", "standard_6x1_limb", "precordial_6x1"],
            7: ["standard_6x2_with_r1"],
            12: ["standard_12x1", "cabrera_12x1"],
        }

        # Try preferred layouts for this row count
        preferred = row_preferences.get(rows, [])
        for layout_name in preferred:
            if layout_name in layouts:
                return layout_name

        # Fallback: find layout with closest total_rows count
        best_match: Optional[str] = None
        best_diff = float('inf')
        for name, desc in layouts.items():
            total_rows = desc.get("total_rows", desc["layout"]["rows"])
            diff = abs(total_rows - rows)
            if diff < best_diff:
                best_diff = diff
                best_match = name

        return best_match or list(layouts.keys())[0]
    ...
    def __call__(
        ...
        threshold: float = 0.5,  # Lowered from 0.8 to detect more labels
        mv_per_mm: float = 0.1,
        ...
        if "layout" not in match:
            # Select layout based on detected row count instead of alphabetical first
            fallback_layout = self._select_layout_by_row_count(rows_in_layout, layouts)
            print(f"No matching layout found, using row-based fallback: {fallback_layout} (for {rows_in_layout} rows)")
            match["layout"] = fallback_layout
